...
from app.models import Student, Group, SupervisorOfStudentGroup, ProjectCategories
...
class TestStudentProfileView:
    ...
    def student_user(self):
        return User.objects.create_user(
            username="student1", password="strongpass123", user_type="student"
        )
    ...
    def groupmate_group(self, student, project_category):
        mate_user = User.objects.create_user(
            username="student2", password="strongpass123", user_type="student"
        )
    ...
    def test_student_profile_with_group_ids(
        self, api_client, student_user, student, supervisor_group, groupmate_group
    ):
        api_client.force_authenticate(user=student_user)
        url = reverse("student-profile")
        response = api_client.get(url)
